...
1. __Price-weighting: WWW comprises $$\frac{2}{3}$$ of the DJIA, even though XYZ is the same size as WWW; assuming constant shares outstanding:
...
1. __Transparency:__ Unlike for other indices, there is no clear set of rules for selecting the index components (e.g., "select the top 3,000 largest publicly held companies incorporated in the U.S. based on market capitalization")
...
In order to get a sense for the impact of price-weighting vs. cap-weighting[^2], let's benchmark the DJIA against a new, cap-weighted index composed of the same stocks (which I'll call the Dow Jones Market Cap Average, or _DJMCA_)[^3]. Figure 1 compares the cap-weighted DJMCA against the price-weighted DJIA and the S&P 500 (you can also click on the legend to view the cap-weighted Russell 2000). The S&P 500 includes many times more stocks than the DJIA and it is cap-weighted[^4], so it serves as an informative benchmark.
...
The DJMCA shows lower returns than does the DJIA -- if weighted by market cap, the DJIA would have shown only a 23% gain since September 2012, whereas the actual DJIA shows a gain of 32%. The S&P 500, meanwhile, gained 44% over the same period. Taken together, these findings indicate the DJIA components are, unsurprisingly, a poor reflection of what's going on in the broader market; additionally, the distortion introduced by price-weighting makes the index look like a better reflection of the market (i.e., closer to the S&P 500) than it actually is. Two wrongs (i.e., limited stock selection and price weighting) are jammed together into one index to create something that looks right at first glance.

Figure 1 tells us what the DJIA would look like if cap-weighted, but only tells us how much the DJIA changed vs. how much it should have changed. It requires a reference point (I anchored the index changes to an arbitrary date in 2012) -- you can't look at any single day and say how distorting the DJIA price-weighting is that day. It is also not clear from Figure 1 what is driving any disparity observed. To better answer the question at hand, I introduce...

## The Dow Jones Distortion Index
In order to measure the extent to which price-weighting distorts the DJIA at any given point in time, I created the Dow Jones Distortion Index ("DJDI"), which compares each stock's contribution to the DJIA against its contribution to a theoretical cap-weighted index composed of the same stocks. The DJDI represents the total deviance of each stocks' price weight from its market cap weight (additional details on methodology [here](http://eightportions.com/references/DJDI_methodology)). The DJDI is 0% when the DJIA is at its best and 100% when the DJIA is at its worst[^5].
...
1. Index component substitution (i.e., stocks formerly in the index are replaced with new ones)
...
* __March 19, 2015:__ T is swapped out for AAPL, and V issues a 4:1 stock split
    ...
    * Prior to the stock split, T accounted for 9.5% of the index by price, and only 3.3% of the combined market cap; after the stock split it accounts for 2.5% of the total price and 3% of the combined market cap
...
One of the nice things about the DJDI is it allows us to see which stocks are driving these distortion spikes. The contribution of each stock to the DJDI is shown below (the sum of the contribution of all stocks is equal to the DJDI). Each of the major shifts in the DJDI described above, can be explored at the individual stock level.
...
[^5]: A DJDI of 0% indicates that the DJIA is market cap weighted, and a DJDI of 100% indicates that the DJIA is measuring only things other than the stocks comprising the DJIA.