...
In this post I introduce _Trump bot_, a Long Short Term Memory (LSTM) Recurrent Neural Network (RNN) that generates new Donald Trump speeches. _Trump bot_ is trained on my [compilation](https://github.com/rtlee9/Trump-bot/raw/master/data/trump/input_norm_caps/input.txt) of transcripts of speeches delivered by Trump throughout this last year. Though prone to grammar mistakes and rare but sudden changes of opinion, _Trump bot's_ speeches bear a strong resemblance to those of its namesake.
...
* [Evaluation: out-of-sample comparisons](#eval)
...
The word-level model has the advantage of not needing to learn how to spell, as each prediction is constrained to a finite vocabulary. As a result, it can spend its time learning higher-level concepts, like how to compose sentences with proper grammar. A second advantage of the word-level model is that it can be initialized with pre-trained word embeddings, giving it a head start in the training process[^1]. To my knowledge, there isn't any direct scientific support for these assertions, but experimental testing for this post broadly supports my intuition (additional details below).

While these characteristics make the word-level model a good candidate for generating meaningful sentences, I found the character-level model to be better at _completing_ sentences when provided with a small seed. As a result, _Trump bot_ blends the two models: it uses the character-level model to complete the first sentence based on the seed provided, and uses the resulting sentence as the seed for the word-level model.

On the human side of the equation, Trump's speaking style has been described by linguists as [aphasic and unstructured](http://languagelog.ldc.upenn.edu/nll/?p=20490#more-20490), [repetitive](http://languagelog.ldc.upenn.edu/nll/?p=22691), [grammatically simple](https://arxiv.org/pdf/1603.05739v1.pdf), and even [feminine](http://www.politico.com/magazine/story/2016/10/trump-feminine-speaking-style-214391). Mark Liberman [points out](http://languagelog.ldc.upenn.edu/nll/?p=20492) that these observations are more prominent when reading his transcribed speeches than when listening to them in audio, likely one of the reasons his supporters find him so compelling. While Trump's speaking style presents certain modeling challenges, like getting "stuck" in syntactic loops, his simple grammar and vocabulary make his speeches a good (or interesting, at the very least) candidate for modeling.

### The training
For both the character-level model and the word-level model I scripted a training grid to vary the LSTM's RNN size, number of layers, dropout rate and sequence length. I selected the top model based on cross-validation results, manually checked its sampled text quality, and then extended training with additional epochs as necessary.

I found that the character-level model was better at completing sentences from short phrases, while the word-level model was better at stringing together multiple words into semi-coherent sentences. As a result, I created a short python script to blend samples from the two models together, per my description above.
...
Below are samples from two speeches: one from a real Trump speech and one written by _Trump bot_.
...
Sample A came from Trump's [December 21<sup>st</sup> speech in Grand Rapids, MI](https://www.youtube.com/watch?v=hXeFm0OecKk) (sourced from [Mark Liberman](http://languagelog.ldc.upenn.edu/nll/?p=23057)) and Sample B was produced by _Trump bot_, seeded by the phrase "I will build a wall." Sample A was not in the training set for the _Trump bot_ model. I introduced newlines to both speeches at intuitive points in the speeches. Below, I outline a few of the most notable similarities and differences between the two speeches.

### Key similarities
In spite of these flaws, _Trump bot's_ style is remarkably similar, in my opinion, to that of the real speeches. Here are a few examples with supporting excerpts:
...
<ul>
  ...
  <li><i>"I'm one of the most important, you know, in the whole world"</i></li>
</ul>
</td>
</tr>
</table>
...
Similarity of style alone is not difficult to achieve -- I could easily generate speeches similar in style by copying and pasting sentences from the real transcripts in random order. What is impressive is that _Trump bot_ generates entirely new phrase and sentences: 51% of all consecutive three-word sequences and 76% of all consecutive four-word sequences are not found in Trump's real life transcripts (see [Figure 1](#fig1) for more details).

#### Figure 1: Generated sequence novelty {#fig1}
{% include percent_new_list.html %}

Lastly, _Trump bot_ more often than not produces _content_ that Trump would undoubtedly support, if he hasn't done so already. Here are a few examples:
...
### Key differences
There are a few notable differences between these two speeches. The biggest difference is that the real speech is clearly working towards something, though it's not necessarily clear what that something is until the third paragraph. The purpose of the speech is to tell the audience that Trump can bring jobs back to America. There are many side notes and asides[^3], but there is a clear central point to it all. _Trump bot_, however, doesn't have a consistent argument throughout; instead, it produces a mixed-content speech that, on average,  more closely resembles the "typical" Trump speech.

A second noticeable difference is that _Trump bot_, unsurprisingly, makes mistakes more frequently than the real Trump. For example, it might make the grammatical mistake of using _no_ instead of _not_: "I think it was no beautiful". Trump's speech transcripts often contain similar mistakes (though arguably to a lesser extent), which undoubtedly contributes to such mistakes, as _Trump bot_ learns from these transcripts. Less forgivable, however, _Trump bot_ sometimes gets things backwards:
...
Future work might focus on enhancing these models by testing the following suggestions:

* __Document-specific embeddings:__ add a unique document ID to the input layer to capture speech-specific themes
* __Speech parsing and separation:__ optimize text normalization to retain important features (e.g., quotation marks), while minimizing unnecessary noise
* __Additional hyperparameter tuning:__ test additional hyperparameter combinations
* __Add data:__ Add data from recent speeches and other forms of media (e.g., Twitter)

## Selected _Trump bot_ samples {#samples}

I've selected a few of my favorite samples from the model. I've <u>underlined</u> some of the more realistic parts of the samples, and I've also <mark>highlighted</mark> a few of the more memorable quotes for your enjoyment.


Similar to the sample analyzed above these samples tend to capture the style and key ideas of the real speeches, while often lacking in continuity and sometimes making grammar mistakes. One thing that is more apparent in these samples is that  _Trump bot_ has started to learn the importance of story telling, often recounting previous conversations with (imaginary) characters: key phrases are often preceded by "I said", or "He said.
...
## Credits, inspiration and similar projects {#resources}
The _Trump bot_  models are a fork of Lars Hiller Eidnes' [word-rnn](https://github.com/larspars/word-rnn), which is based on Andrej Karpathy's [char-rnn](https://github.com/karpathy/char-rnn). Below are a few similar projects I found in building this model:
...
[^2]: Individual training runs may yield slightly different results due to randomness introduced as part of the training (e.g., minibatch selection, random weight / bias initialization)
[^3]: Such as the claim that the CEO of Ford is a "nice guy", which was immediately followed by a retraction that same claim
